...
function setFstDropdown() {
    ...
    function changeMultipleSelect(select, event, dd, selectAll, opened) {
        ...
        if (opened)
            dd.querySelector(".fstselected").textContent = length == 1 ? event.target.textContent : length + " serial number selected";
            ...
            if (select.multiple) {
                ...
                for (var s in select.options)
                    if (select.options.hasOwnProperty(s) && select.options[s].selected == true)
                        count++;
                text = count == 1 ? sOption.text : count + " serial number selected";
            }
